...
import { FiAlertCircle, FiLock, FiRefreshCw, FiShield } from "react-icons/fi";
import Logo from "@/common/logo";
import { isAuthenticated } from "@/lib/auth";
import { passwordProblem } from "@/lib/password";
import type { ResetPasswordRequest } from "@/models/request/auth-request";
...
export default async function ResetPasswordPage({
  ...
  async function resetPassword(formData: FormData) {
    ...
    const back = `/auth/reset-password?token=${encodeURIComponent(request.token)}`;

    if (!request.token) redirect("/auth/reset-password?error=token");

    const problem = passwordProblem(request.password);
    if (problem) redirect(`${back}&error=${encodeURIComponent(problem)}`);

    if (request.password !== request.confirmPassword) {
      redirect(`${back}&error=${encodeURIComponent("The two passwords do not match.")}`);
    }

    // TODO: verify the single-use token, write the new credential, revoke sessions.
          ...
          <div className="text-center">
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-2">
              <span className={chipCls}>
                <FiLock size={14} /> 12 characters min
              </span>
              <span className={chipCls}>
                <FiRefreshCw size={14} /> Single use
              </span>
              <span className={chipCls}>
                <FiShield size={14} /> Signs out other sessions
              </span>
            </div>
          </div>

          {token ? (
            <form action={resetPassword} className="mt-10">
              ...
              <ul className="mt-4 flex flex-wrap justify-center gap-x-5 gap-y-1 text-sm text-gray-500 dark:text-neutral-400">
                {RULES.map((r) => (
                  <li key={r}>· {r}</li>
                ))}
              </ul>
            ...
            <p className="mt-10 rounded-xl border border-state-300 bg-state-50 px-4 py-3 text-center text-base text-state-800 dark:border-state-700 dark:bg-state-900/20 dark:text-state-200">
              ...
              <Link href="/auth/forget-password" className="font-bold underline">
                forgotten password
              </Link>{" "}
              screen and open the link from your email.
            </p>
          )}
