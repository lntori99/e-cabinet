...
interface LoaderProps {
  size?: number; // size of spinner
  fullscreen?: boolean; // center it in the screen
}

const Loader: React.FC<LoaderProps> = ({ size = 25, fullscreen = false }) => {
  return (
    <div
      className={`flex justify-center items-center ${
        fullscreen
          ? "fixed inset-0 z-50 bg-transparent dark:bg-neutral-800"
          : ""
      }`}
      role="status"
    >
      <svg
        aria-hidden="true"
        className="animate-spin text-neutral-300 dark:text-neutral-700 fill-neutral-800 dark:fill-neutral-200"
